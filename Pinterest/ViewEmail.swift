...
class ViewEmail: UIViewController {
    ...
    @objc func handleButton(){
        if let registerVal = register {
            ...
            case .password:
                let email = ViewEmail()
                if let usuarioInstance = usuario {
                    usuarioInstance.password = emailTextField.text
                    email.usuario = usuarioInstance
                }
                email.register = .age
                self.navigationController?.pushViewController(email, animated: true)
            case .age:
                if let usuarioInstance = usuario {
                    usuarioInstance.age = emailTextField.text
                    if let email = usuarioInstance.email, let pass = usuarioInstance.password, let age = usuarioInstance.age{
                        print(email)
                        print(pass)
